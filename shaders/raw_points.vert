// Vertex shader: animates the 10k random points through three sequences
// (dust cloud, weird circle, torus) blended together over time + scroll.
...
vec3 swap_spark_vec3(vec3 a, vec3 b, float anim_time, float swp_time, float swap_area, float delta_time) {
    ...
    return mix(mix(a, 2.*b, smoothstep(spark_swp_time-delta_time, spark_swp_time, anim_time)),
               mix(2.*b, b, smoothstep(spark_swp_time-delta_time, spark_swp_time, anim_time)),
...
vec3 project_onto_torus(vec3 p, float pert) {
    ...
    pp = normalize(pp);
    vec3 r = normalize(p-pp);
    float pert_pow = pow(clamp(pert, 0., 0.3)/0.3, 32.);
    return pp + (0.3 + 0.3*length(sin(p * 1.*vec3(1., 5., 3.)*pert_pow + 0.02*vec3(17., 58., 3.6) * pert_pow))) * r;
}
...
vec3 torus(vec3 vertex_pos, float anim_time) {
    ...
    torus_pos.yz *= rotate(20. * (1.-abs(anim_time-1.)));
    torus_pos.xy *= rotate(14. * 0.2 * (1.-abs(anim_time-1.)));
